w.ui.View = function () {
	...
	return w.inherit({
		...
		off: function (type, fn) {
			...
			if (fn) {
				this.eventArr[type] = w.without(this.eventArr[type], fn);
			} else {
				this.eventArr[type] = [];
			}
		},
		...
		setOption: function (options) {
			for (var k in options) {
				if (k == 'datamodel') {
					w.extend(this.datamodel, options[k]);
					continue;
				}
				this[k] = options[k]
			}
		...
		render: function (data, callback) {
			...
			if (data) {
				html = w.template(this.template, data);
			}
		...
		bindEvents: function () {
			...
			if (!(events || (events = w.result(this, 'events')))) return this;
			this.unBindEvents();
			...
			for (key in events) {
				method = events[key];
				if (!w.isFunction(method)) method = this[events[key]];
				if (!method) continue;
